                    ...
                    @if ( !empty($permissions))
                        ...
                        <th><div class="row">
                                <div class="col">Acciónes<div>
                                    @foreach ($sistemas as $_sh => $_sv)
                                    @if( !empty($permissions[$_sv->sys_id]['create']))
                                    <a type="button" class="btn btn-success btn-sm m-1 p-1 rounded" href="{{route($permissions[$_sv['sys_id']]['create'])}}">Agregar</a>
                                    @break
                                    @endif
                                    @endforeach
                            </div>
                            ...
                            @if ( !empty($permissions))
                                <td>
                                    <div class="btn-group float-right">
                                        @if ( !empty($permissions[$row->nivel]['update']))
                                            <a type="button" class="btn-primary btn-sm m-1 p-1 rounded" href="{{route($permissions[$row->nivel]['update'],$row['id'])}}">Editar</a>
                                        @endif
                                        @if ( !empty($permissions[$row->nivel]['delete']))
                                        {{ html()->modelForm($row,'delete',route($permissions[$row->nivel]['delete'],$row->id))->id($row->id)->open() }}
                                            @if(($row['deleted'])==0)
                            ...
                            @if ( !empty($permissions))
                                <td>
                                    <div class="btn-group float-right">
                                        @if ( !empty($permissions[$row->nivel]['update']))
                                            <div class="col">
                                                <a type="button" class="btn btn-primary btn-sm m-1 p-1 rounded" href="{{route($permissions[$row->nivel]['update'],$row->id)}}">Editar</a>
                                            </div>
                                        @endif
                                        @if ( !empty($permissions[$row->nivel]['delete']))
                                        {{ html()->modelForm($row,'delete',route($permissions[$row->nivel]['delete'],$row->id))->id($row->id)->open() }}
