...
@endphp
    ...
    <div>
                ...
                @if (!empty($table_config) && !empty($table_config['actions']) && !empty($table_config['actions']['extras_accordion']) )
                    <div id="{{ $_data_table_id }}-filter-accordion-wrapper" class="dataTables-filter-wrapper no-outer-spacing accordion">
                        <div class="card">
                            ...
                            <div role="button" class="card-header p-0 m-0" id="{{ $_data_table_id }}-filter-accordion-heading">
                                <section class="mb-0 mt-0">
                                    <div role="menu" class="row " data-bs-toggle="collapse" data-bs-target="#{{ $_data_table_id }}-filter-accordion" aria-expanded="true" aria-controls="{{ $_data_table_id }}-filter-accordion">
                                        <div class="size-120 col"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-chevron-down"><polyline points="6 9 12 15 18 9"></polyline></svg> &nbsp; Filtrar: </div>
                                        <div class="icons col"></div>
                                    </div>
                                </section>
                            </div>

                            <div id="{{ $_data_table_id }}-filter-accordion" class="collapse show" aria-labelledby="{{ $_data_table_id }}-filter-accordion-heading" data-bs-parent="#{{ $_data_table_id }}-filter-accordion-wrapper">
                                <div class="card-body">
                @else
                    <div class="dataTables-filter-wrapper card text-dark">
                        <div class="card-body">
                                ...
                                @elseif ( $_s_c_val['type'] == 'select' )
                                    <select class="form-control simple-select" data-filter-column="select" data-filter-column-name="{{ $_h_key }}">
                                        ...
                                        @foreach ( $_s_c_val['options'] as $_t_f_opt_key => $_t_f_opt_val )
                                            <option value="{{ $_t_f_opt_val }}">{{ $_t_f_opt_val }}</option>
                                        @endforeach
                                    </select>
        ...
        {{-- Tabla de datos --}}
            <table id="{{ $_data_table_id }}" class="table table-striped dt-table-hover text-wrap" style="width:100%;">
                ...
                <thead>
                        ...
                        @if (!empty($permissions))
                            <th>
                                ...
                                {{-- <div class="col"><a type="button" class="btn btn-success btn-sm m-1 p-1 rounded" href="{{route($permissions['create'])}}">Agregar</a></div> --}}
                                <div class="col"><a type="button" class="btn btn-success btn-sm m-1 p-1 rounded" data-bs-toggle="modal" data-bs-target="#crearficha">Agregar</a></div>
                                @endif
...
<!-- Modal Nuevo -->
    @livewire('monitoreo.asm.fichas-form')
